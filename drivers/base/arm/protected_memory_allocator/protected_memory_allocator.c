...
static int protected_memory_allocator_probe(struct platform_device *pdev)
{
	...
	if (has_acpi_companion(&pdev->dev)) {
		...
		rmem_base = res->start;
		rmem_size = (res->end - res->start + 1) >> PAGE_SHIFT;
	} else {
...
static const struct acpi_device_id protected_memory_allocator_acpi_ids[] = {
	{ .id = "CIXHA012", .driver_data = 0 },
	{ /* sentinel */ }
};
MODULE_DEVICE_TABLE(acpi, protected_memory_allocator_acpi_ids);
...
static struct platform_driver
	protected_memory_allocator_driver = { .probe = protected_memory_allocator_probe,
					      ...
					      .driver = {
						      ...
						      .acpi_match_table = ACPI_PTR(protected_memory_allocator_acpi_ids),
					      } };
