...
int power_control_init(struct kbase_device *kbdev)
{
	...
	if (has_acpi_companion(kbdev->dev)) {
		char *clk_names[BASE_MAX_NR_CLOCKS_REGULATORS] = {"gpu_clk_core", "gpu_clk_stacks"};
		...
		*/
		...
		kbdev->nr_clocks = i;
		dev_dbg(&pdev->dev, "Clocks probed: %u, err=%d\n", kbdev->nr_clocks, err);
		...
		for (i = 0; i < MAX_GPU_BACKUP_CLOCKS; i++) {
			...
			if (err) {
				dev_err(&pdev->dev, "Failed to prepare and enable backup clock (%d)\n", err);
				clk_put(kbdev->backup_clocks[i]);
				break;
			}
		}
		...
		for (i = 0; i < MAX_GPU_BACKUP_CLOCKS; i++) {
			kbdev->backup_clocks[i] = of_clk_get_by_name(kbdev->dev->of_node, backup_clk_names[i]);
			if (IS_ERR(kbdev->backup_clocks[i])) {
				err = PTR_ERR(kbdev->backup_clocks[i]);
				kbdev->backup_clocks[i] = NULL;
				break;
...
extern bool sky1_power_timer_enabled;

/* Get the value of sky1 power model timer switch*/
static ssize_t sky1_power_timer_show(struct device *dev, struct device_attribute *att, char *buf)
{
    return sprintf(buf, "%d\n", sky1_power_timer_enabled);
}

/* Set the value of sky1 power model timer switch*/
static ssize_t sky1_power_timer_store(struct device *dev, struct device_attribute *attr,
						const char *buf, size_t count)
{
	struct kbase_device *kbdev = dev_get_drvdata(dev);
	unsigned int val;
	...
	if (val == 1 && !sky1_power_timer_enabled) {
		sky1_power_timer_enabled = true;
		hrtimer_start(&kbdev->sky1_power_timer,
				HR_TIMER_DELAY_MSEC(PM_POWER_MODEL_SAMPLE_INTERVAL_MS), HRTIMER_MODE_REL);
	} else if (val == 0 && sky1_power_timer_enabled) {
		sky1_power_timer_enabled = false;
...
static DEVICE_ATTR_RW(sky1_power_timer);
...
static struct attribute *kbase_attrs[] = {
	...
	&dev_attr_sky1_power_timer.attr,
#endif /* !MALI_USE_CSF */
