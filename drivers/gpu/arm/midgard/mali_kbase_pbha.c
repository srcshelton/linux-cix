...
void kbase_pbha_write_settings(struct kbase_device *kbdev)
{
	...
	if (kbasep_pbha_supported(kbdev)) {
		uint i;
		/* In default memory group manager implementation, IMPORTED_MEMORY_ID is set to 15, and
		 * this makes the PBHA bit of all imported dmabuf set to 15. To make all GPU imported
		 * dmabufs which shared with devices in MMHUB never alloc system level cache, set the
		 * SYSC_PBHA_OVERRIDE for indices 15 to 0x22(NEVER_ALLOC).
		 */
		kbase_reg_write32(kbdev, GPU_SYSC_PBHA_OVERRIDE_OFFSET(3), 0x22000000);
...
static int kbase_pbha_read_int_id_override_property(struct kbase_device *kbdev,
						    const struct fwnode_handle *pbha_node)
{
	u32 elements_data[GPU_SYSC_ALLOC_COUNT * sizeof(u32) * SET_SIZE];
	int sz, i;
	bool valid = true;

	sz = fwnode_property_count_u32(pbha_node, "int-id-override");

	if (sz == -EINVAL) {
		/* There is no int-id-override field. Fallback to int_id_override instead */
		sz = fwnode_property_count_u32(pbha_node, "int_id_override");
	}
	if (sz == -EINVAL) {
	...
	if (sz <= 0 || (sz % SET_SIZE != 0)) {
		dev_err(kbdev->dev, "Bad %s format: pbha.int_id_override\n", 
				is_of_node(pbha_node) ? "DTB" : "ACPI");
		return -EINVAL;
	}
	if (fwnode_property_read_u32_array(pbha_node, "int-id-override", elements_data, (size_t)sz) != 0) {
		/* There may be no int-id-override field. Fallback to int_id_override instead */
		if (fwnode_property_read_u32_array(pbha_node, "int_id_override", elements_data,
					       (size_t)sz) != 0) {
			dev_err(kbdev->dev, "Failed to read %s pbha.int_id_override\n",
					is_of_node(pbha_node) ? "DTB" : "ACPI");
			return -EINVAL;
		}
	}

	for (i = 0; valid && i < sz; i = i + SET_SIZE) {
		unsigned int rdset = SYSC_ALLOC_R_SYSC_ALLOC0_GET(elements_data[i + 1]);
		unsigned int wrset = SYSC_ALLOC_W_SYSC_ALLOC0_GET(elements_data[i + 1]);
		valid = valid &&
			(kbase_pbha_record_settings(kbdev, false, elements_data[i], rdset, wrset) == 0);
		if (valid)
...
/* Read pbha from DTB or ACPI tables */
int kbase_pbha_read(struct kbase_device *kbdev)
{
#if MALI_USE_CSF
	const struct fwnode_handle *pbha_node;
	int err;
	...
	pbha_node = device_get_named_child_node(kbdev->dev, "pbha");
	if (!pbha_node)
		return 0;
	...
	if (!has_acpi_companion(kbdev->dev)) {
		err = kbase_pbha_read_propagate_bits_property(kbdev, to_of_node(pbha_node));
	}
	...
	if (err < 0)
  	...
  	if (!has_acpi_companion(kbdev->dev)) {
		err = kbase_pbha_read_mma_wa_id_property(kbdev, to_of_node(pbha_node));
        }
